/**
 * Create Dean User Script
 * Creates a default dean user or resets dean password
 */
...
async function createDean() {
  console.log('🔐 Create/Reset Dean User\n');
  
  try {
    const email = await question('Dean email (@ku.th or @live.ku.th): ');
    ...
    const password = await question('Dean password (min 8 characters): ');
    ...
    const result = await transaction(async (client) => {
      ...
      if (roleResult.rows.length === 0) {
        throw new Error('DEAN role not found. Run migrations first!');
      }
      
      const deanRoleId = roleResult.rows[0].id;
      
      // Assign DEAN role (if not already assigned)
      await client.query(
