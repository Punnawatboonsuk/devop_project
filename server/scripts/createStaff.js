 ...
 * Creates a default staff user or resets staff password
 */
...
async function createStaff() {
  console.log('🔐 Create/Reset Staff User\n');
  
  try {
    const email = await question('Staff email (@ku.th or @live.ku.th): ');
    ...
    const password = await question('Staff password (min 8 characters): ');
    ...
    const result = await transaction(async (client) => {
      ...
      if (roleResult.rows.length === 0) {
        throw new Error('STAFF role not found. Run migrations first!');
      }
